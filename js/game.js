...
function clearInputState() {
  ...
  _touchActions.interact = false;
  _touchActions.drop = false;
}
...
function findSafeItemDropPosition(item, room, preferredX, preferredY) {
  const clampX = (x) => clamp(x, 8, W - item.w - 8);
  const clampY = (y) => clamp(y, 8, H - item.h - 8);
  const solids = room?.solidRects ? room.solidRects() : [];
  const isBlocked = (x, y) => {
  ...
  const startX = clampX(preferredX);
  const startY = clampY(preferredY);
...
class Bat {
  ...
  _maybeDrop(world, dt) {
    ...
    const room = world.rooms[this.roomId];
    if (!room) return;
    const target = findSafeItemDropPosition(
      this.carrying,
      room,
      this.x + this.w + 6,
      this.y + (this.h - this.carrying.h)/2
    );
    if (!target) return;
  ...
  draw() {
    const r = this.rect();
    drawImageOrFallback(IMAGES.bat, r, "#6c7a89", "BAT");
...
function buildGauntletLayout(world) {
  ...
  addRoom("fortGate", "Fortress Gate", {
    neighbors: { west: "fields", east: "fortHall" },
    bgColor: "#2f261d"
  }, (room) => {
    addBoundaryWalls(room, { open: { west: true, east: true } });
